...
class DayEnergyFlowAnalyzer extends IPSModule
{
    ...
    public function ExportDetails(){ $j=GetValueString($this->GetIDForIdent('ResultDetailsJSON')); if($j===''){ echo 'data:text/plain;charset=utf-8,'.rawurlencode('Keine Details vorhanden.'); return;} $rows=json_decode($j,true); if(!is_array($rows)||empty($rows)){ echo 'data:text/plain;charset=utf-8,'.rawurlencode('Details-Format ungültig.'); return;} $keys=array_keys($rows[0]); $csv=','.implode(',',array_map(function($k){return '"'.str_replace('"','""',$k).'"';},$keys))."\n"; foreach($rows as $r){ $line=[]; foreach($keys as $k){ $v=$r[$k]; if($v===null)$v=''; $line[]='"'.str_replace('"','""',(string)$v).'"'; } $csv.=','.implode(',',$line)."\n"; } echo 'data:text/csv;charset=utf-8,'.rawurlencode($csv); }
    ...
    //public function OpenHelp(){ $f = __DIR__ . '/docs/help.html'; $h='<!doctype html><html><head><meta charset="utf-8"><title>Day Energy Flow Analyzer – Hilfe</title><meta name="viewport" content="width=device-width, initial-scale=1"><style>body{font:14px/1.55 -apple-system,system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;padding:24px;color:#1f2328;background:#fff}h1,h2,h3{margin:1.2em 0 .4em} pre{background:#f6f8fa;padding:12px;border:1px solid #e1e4e8;border-radius:6px;overflow:auto}code{font-family:ui-monospace,Consolas,Menlo,monospace;background:#f6f8fa;padding:1px 4px;border-radius:4px;border:1px solid #e1e4e8}ul{padding-left:1.3em} hr{border:none;border-top:1px solid #e1e4e8;margin:1.25em 0}</style></head><body>'; $h.='<h1>Day Energy Flow Analyzer – README</h1>'; if(file_exists($f)){ $md=@file_get_contents($f); if($md!==false && $md!==''){ $txt=str_replace(["\r\n","\r"],"\n",$md); $txt=preg_replace('/^######\s+(.*)$/m','<h6>$1</h6>',$txt); $txt=preg_replace('/^#####\s+(.*)$/m','<h5>$1</h5>',$txt); $txt=preg_replace('/^####\s+(.*)$/m','<h4>$1</h4>',$txt); $txt=preg_replace('/^###\s+(.*)$/m','<h3>$1</h3>',$txt); $txt=preg_replace('/^##\s+(.*)$/m','<h2>$1</h2>',$txt); $txt=preg_replace('/^#\s+(.*)$/m','<h1>$1</h1>',$txt); $txt=preg_replace_callback('/```([\s\S]*?)```/m',function($m){return '<pre><code>'.htmlspecialchars($m[1],ENT_QUOTES|ENT_SUBSTITUTE|ENT_HTML5).'</code></pre>';},$txt); $txt=preg_replace_callback('/`([^`]+)`/',function($m){return '<code>'.htmlspecialchars($m[1],ENT_QUOTES|ENT_SUBSTITUTE|ENT_HTML5).'</code>';},$txt); $txt=preg_replace('/^\s*[-*]\s+(.*)$/m','<li>$1</li>',$txt); $txt=preg_replace('/(<li>.*<\/li>\n?)+/m',function($m){return '<ul>'.str_replace("\n",'', $m[0]).'</ul>\n';},$txt); $txt=preg_replace('/\*\*(.*?)\*\*/s','<strong>$1</strong>',$txt); $txt=preg_replace('/\*(.*?)\*/s','<em>$1</em>',$txt); $parts=array_map('trim', explode("\n\n",$txt)); foreach($parts as $p){ if($p==='')continue; if(preg_match('/^<h[1-6]>/', $p) || preg_match('/^<ul>/', $p) || preg_match('/^<pre>/', $p)) $h.=$p; else $h.='<p>'.nl2br($p).'</p>'; } } else $h.='<p><b>README.md ist leer oder konnte nicht gelesen werden.</b></p>'; } else $h.='<p><b>README.md nicht gefunden.</b></p>'; $h.='</body></html>'; echo 'data:text/html;charset=utf-8,'.rawurlencode($h); }

    public function OpenHelp()
    {
        $file = __DIR__ . '/docs/help.html';

        if (!file_exists($file)) {
            echo 'data:text/plain;charset=utf-8,' . rawurlencode('help.html nicht gefunden.');
            return;
        }

        $html = file_get_contents($file);
        echo 'data:text/html;charset=utf-8,' . rawurlencode($html);
    }
